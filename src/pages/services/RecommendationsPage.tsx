...
export default function RecommendationsPage() {
  ...
  const { data: services, isLoading } = useRecommendations(cityId, user?.id || null, 50);
  ...
  const title = user ? (isRTL ? "مقترح لك" : "For You") : (isRTL ? "الأكثر شعبية" : "Most Popular");
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          {selectedCityName ? (
            <div className="text-xs text-muted-foreground">
              {isRTL ? "المدينة:" : "City:"} <span className="text-foreground">{selectedCityName}</span>
            </div>
          ) : null}
        </div>
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {Array.from({ length: 9 }).map((_, i) => (
              <div key={`recommendations-loading-${i}`} className={`${HUB_CARD_BASE} bg-card overflow-hidden`}>
