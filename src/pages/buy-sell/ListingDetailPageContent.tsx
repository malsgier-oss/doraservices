...
interface ListingDetailPageContentProps {
  listingId?: string;
  categorySlug?: string;
}
...
export default function ListingDetailPageContent({ listingId, categorySlug }: ListingDetailPageContentProps) {
  ...
  const handleListingClick = (clickedListingId: string) => {
    if (categorySlug) {
      navigate(`/buy-sell/category/${categorySlug}/listing/${clickedListingId}`);
    } else {
      navigate(`/listings/${clickedListingId}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      {/* 12. Bottom contact bar with Favorite button */}
