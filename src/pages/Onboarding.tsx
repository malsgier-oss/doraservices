    ...
    () => [
      {
        ...
        description: isRTL
          ? "ابحث حسب الفئة والمدينة — كهرباء، سباكة، تكييف…"
          : "Browse by category and city — electrician, plumbing, AC…",
      },
      {
        icon: PhoneCall,
        title: isRTL ? "اتصل مباشرة" : "Call instantly",
        description: isRTL
          ? "لا حجوزات ولا تعقيد — افتح الخدمة واضغط اتصال."
          : "No booking friction — open a service and tap call.",
      },
      {
        icon: ShieldCheck,
        title: isRTL ? "اختر استخدامك" : "Choose your path",
        description: isRTL
          ? "هل تبحث عن خدمة أم تقدم خدمات؟"
          : "Are you looking for a service, or providing services?",
  ...
  const markDoneAndGo = (selectedIntent?: Intent) => {
    ...
    if (chosen === "provider") {
      if (user) {
        ...
        navigate("/auth?tab=signup&intent=provider", { replace: true });
      }
      return;
    }
    ...
    <div className="min-h-screen bg-background flex flex-col" dir={isRTL ? "rtl" : "ltr"}>
      <div className="flex items-center justify-between px-5 pt-5">
        <div className="flex items-center gap-2">
          <img src={doraLogo} alt="Dora" className="w-9 h-9 rounded-2xl object-cover" />
        ...
        <Card className="w-full max-w-md rounded-3xl shadow-card border-border/60">
          <CardContent className="p-8">
            <div className="flex flex-col items-center text-center gap-4">
              ...
              {"isChoice" in current && current.isChoice ? (
                <div className="w-full grid gap-3 mt-2">
                  <button
                    ...
                    className={cn(
                      ...
                      intent === "user" ? "border-primary bg-primary/5" : "border-border bg-background",
                    )}
                  >
                  ...
                  <button
                    ...
                    className={cn(
                      "w-full rounded-2xl border px-4 py-4 text-left transition",
                      intent === "provider" ? "border-primary bg-primary/5" : "border-border bg-background",
                    )}
                  >
                    <div className="flex items-center gap-3">
